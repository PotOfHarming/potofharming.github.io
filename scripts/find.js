...
class Flight {
    constructor(origin, destination, transfers, depart, arrive, daysLater, flightNumbers, price, currency, urls) {
        this.origin = origin;
    ...
    static createFromFare(fare, url = null) {
        return new Flight(
    ...
    static addArrival(url, arr) {
        // Find the part with departureAirportIataCode=
        const insertPos = url.indexOf('&', url.indexOf('departureAirportIataCode='));
        
        if (insertPos === -1) {
            // No other params, just append
            return url + `&arrivalAirportIataCode=${arr}`;
        }

        // Insert &arrivalAirportIataCode=XXX right after departureAirportIataCode=YYY
        return url.slice(0, insertPos) + `&arrivalAirportIataCode=${arr}` + url.slice(insertPos);
    }
}
...
async function fetchSpecificFlight(origin, arrival, sDate, eDate = sDate, sTime = "00%3A00", eTime = "23%3A59", currency = "EUR") {
    ...
    try {
        ...
        return data.fares.map(fl => Flight.createFromFare(fl, FETCH_URL));
    ...
    try {
        ...
        for (let flight of flights.fares) {
            if (betweenAirport) {
                ...
                const f = Flight.createFromFare(flight, FETCH_URL);
                flight_list.push(f);
            }
        }
...
async function findTransferringFlight(origin, arrival, date, currency = "EUR", createCard = false) {
    ...
    for (let { dest: DEST, flight: FLIGHT } of dest_list) {
        ...
        const f = new Flight(
            ...
            currency,
            [...[Flight.addArrival(FLIGHT.urls, FLIGHT.destination)], ...[flight2.urls]]
        );
